...
- Command:
  - `python -m minisgl.benchmark.token_parity --model-path Qwen/Qwen2.5-0.5B-Instruct --backend-a python --backend-b rust_hotpath --max-tokens 16 --token-prompt-count 4 --min-input-len 32 --max-input-len 64 --cuda-graph-max-bs 1 --master-port 2380 --out 0_venkat-worklog/kanban/baselines/latest-token-parity.json`
- Result:
  - `parity_passed=True`
